...
def json2csv(data, key):
    ...
    df[columns].to_csv(f"/tmp/pavel_kond/tmp/{key}.csv", index=False)
...
with DAG(
    ...
    copy_hdfs_task_operator = BashOperator(
        ...
        bash_command="hdfs dfs -mkdir -p /user/shahidkubik/amazon_reviews/staging/ && hadoop fs -put -f /tmp/pavel_kond/tmp/* /user/shahidkubik/amazon_reviews/staging && hdfs dfs -chmod -R 777 /user/shahidkubik/amazon_reviews/"
                     # " && rm -r /tmp/pavel_kond",
    )
