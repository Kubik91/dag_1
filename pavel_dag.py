...
def test_data():
    ...
    with gzip.open('All_Beauty.json.gz', 'rb') as f_in, open('All_Beauty_1.json', 'wb') as f_out_1, open('All_Beauty_2.json', 'wb') as f_out_2:
...
def json2csv(data, key):
    columns = ['overall', 'verified', 'reviewTime', 'reviewerID', 'asin', 'reviewerName',
...
with DAG('kond_dag', schedule_interval='0 * * * *', default_args=default_args, catchup=False) as dag:
    ...
    with TaskGroup('dynamic_tasks_group_load',
        ...
        if keys_list:
            for index, key in enumerate(keys_list):
                ...
                remove_temp_table_hql = """DROP TABLE IF EXISTS {{ params.table_name }};"""
    ...
    with TaskGroup('dynamic_tasks_group_drop_duplicates',
                   prefix_group_id=False,
